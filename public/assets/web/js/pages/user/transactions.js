window.TapSNDRPage = (() => {
    ...
    const loadStats = () => {
        $(ref.transactionsTable.columns([2, 4]).footer()).html('<span class="spinner-border spinner-border-sm text-dark"></span>');
        getStats((data) => {
            $(ref.transactionsTable.column(2).footer()).html(
                '<span class="' +
            ...
            $(ref.transactionsTable.column(4).footer()).html(accounting.formatMoney(data.tickets.amount.total));
